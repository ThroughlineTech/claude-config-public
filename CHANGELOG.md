...
### Fixed — Copilot Plane MCP write tools missing due to 109-tool context truncation

VS Code Copilot discovered all 109 tools exposed by `uvx plane-mcp-server stdio` but silently truncated the list when loading them into the Haiku 4.5 agent context (0.3x token budget). Write tools (`create_work_item`, `create_work_item_relation`, etc.) fell off the end of the list, making `/tn` and similar commands fail with "required tool not found" even though the MCP server was connected and healthy. Fix: `install.sh` now registers a Python stdio proxy (`bin/plane-mcp-proxy`) as the Copilot MCP command instead of `uvx` directly. The proxy intercepts `tools/list` JSON-RPC responses and filters the tool array from 109 → 17 (only the tools actually called by ticket workflow commands), passing all other traffic through unchanged. The `bin/sync-copilot-prompts` pre-flight blockquote gains a third bullet directing users to re-run `install.sh` when write tools are absent despite the server showing as Connected. (CCONF-25)
