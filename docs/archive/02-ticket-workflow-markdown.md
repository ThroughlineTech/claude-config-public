# The Ticket Workflow (Markdown backend — archived)

> **Historical reference.** This is the pre-Plane workflow doc, preserved for projects still on the markdown backend while Plan 3 migrates them. For the current (Plane-backed) workflow, see [`../02-ticket-workflow.md`](../02-ticket-workflow.md). The commands themselves detect which backend a project uses and dispatch accordingly, so this content still applies to any project that has `.claude/ticket-config.md` + `tickets/` but no `.claude/plane-config.md`.
