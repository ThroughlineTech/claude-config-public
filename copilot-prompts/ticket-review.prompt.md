---
mode: agent
description: Generate a human verification checklist for a completed ticket
...
Run every check that can be verified without a human. Read Test, Build, and Lint commands from `.claude/ticket-config.md`. Determine the main branch: read `Main branch` from the config if present, otherwise fall back to `git symbolic-ref refs/remotes/origin/HEAD`. If the result is `master`, warn and suggest `/ticket-install`.

On the ticket's feature branch:

1. **Tests:** run the Test command. Capture pass/fail + counts (passed, failed, skipped).
2. **Typecheck:** run if configured. Capture pass/fail.
3. **Build:** run the Build command. Capture pass/fail + first error if any.
4. **Lint:** run if configured. Capture pass/fail + warning count.
...
Omit any check whose command is not configured.

## Phase 3: Update Ticket

1. Write the `### Automated Checks` section into the ticket's `## Verification Checklist (for human)` area:
...
3. Present the full checklist in the conversation.
