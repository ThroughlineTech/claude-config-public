...
Bring a ticket out of a terminal subfolder (`shipped/`, `deferred/`, `wontfix/`) back into the active set. Use when a shipped change regressed, a deferred ticket's moment has come, or a closed ticket turned out to be real after all.
...
Examples:
- `TKT-014 customer hit the original bug again after the merge, logs attached`
- `TKT-022`  (permitted, but the ticket gets a generic "reopened" note)

## Pre-flight Checks

- `.claude/ticket-config.md` must exist. If not, tell the user to run `/ticket-install` and stop.
- Locate the ticket file: search `{tickets-dir}/shipped/{ID}.md`, `{tickets-dir}/deferred/{ID}.md`, `{tickets-dir}/wontfix/{ID}.md`. If it is already at `{tickets-dir}/{ID}.md` (active), STOP and report "ticket is already active". If not found anywhere, error out.

## Steps

1. **Determine the new active status** based on where the ticket came from and what's filled in:
   - From `shipped/` → `open` (regression; investigation should start over)
...
2. **Translate the reason to English** if one was given. If none was given, use: "reopened on {date}".
...
3. **Update the ticket file in place:**
   ...
   - Append:
     ...
     ```
   - **Do NOT delete** the existing `## Shipped`, `## Deferred`, or `## Closed (wontfix)` sections. Keep them — they're history.
