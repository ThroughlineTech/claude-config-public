---
mode: agent
description: Approve and implement an investigated ticket
...
**Locate the ticket file:** check `{tickets-dir}/{ID}.md` first. If the ticket is in a terminal subfolder (`shipped/`, `deferred/`, `wontfix/`), STOP and tell the user to run `/ticket-reopen {ID}` first.
...
- Ensure working tree is clean (`git status`). If dirty, warn and stop.

## Phase 1: Branch Setup

1. Determine the main branch: read `Main branch` from `.claude/ticket-config.md`. Otherwise fall back to `git symbolic-ref refs/remotes/origin/HEAD`. If the result is `master`, warn the user and suggest running `/ticket-install` to migrate.
...
3. **Write clean code:** follow existing patterns, use existing interfaces, handle errors at system boundaries.
4. **Check off each plan item** in the ticket as you complete it.

## Phase 3: Testing

1. **Add tests** for every new function/method following the project's existing test conventions. Test happy path, error cases, and relevant edge cases.
...
3. **Run build** using the Build command:
   - Must complete cleanly with zero errors.
   - If no Build command is configured, skip and note it.
...
**Files Changed section:** list every file created or modified with a one-line description.
